...
async function main() {
    ...
    try {
        ...
        await monitorListingsUsingEventEmitter(client, 30000000, pipeline);
...
async function monitorListingsUsingEventEmitter(client, timeInMs = 60000, pipeline = []) {
    ...
    changeStream.on('change', (next) => {
        console.log(next);
        // console.log(next.fullDocument.results[0].audio)
        io.emit("message",next.fullDocument.results[0])
    });
